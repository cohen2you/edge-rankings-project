...
export function parseExcelFile(file: File): Promise<{ data: EdgeRankingData[]; rankingCategory: string }> {
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        console.log('=== DETECT RANKING CATEGORY ===');
        const headers = jsonData[0] as string[];
        const rankingCategory = detectRankingCategory(headers, file.name);
        console.log('Detected ranking category:', rankingCategory);
        
        console.log('=== PROCESS EXCEL DATA START ===');
        const processedData = processExcelData(jsonData);
        console.log('=== PROCESS EXCEL DATA COMPLETE ===');
        console.log('Processed data length:', processedData.length);
        resolve({ data: processedData, rankingCategory });
      } catch (error) {
...
export function detectRankingCategory(headers: string[], filename?: string): string {
  const headerText = headers.join(' ').toLowerCase();
  const filenameText = filename ? filename.toLowerCase() : '';
  const combinedText = `${headerText} ${filenameText}`;
  
  console.log('=== RANKING CATEGORY DETECTION ===');
  console.log('Headers:', headers);
  console.log('Filename:', filename);
  console.log('Filename (lowercase):', filenameText);
  console.log('Combined text for detection:', combinedText);
  console.log('Contains "quality":', combinedText.includes('quality'));
  console.log('Contains "quality_percentile":', combinedText.includes('quality_percentile'));
  
  // Check for specific ranking categories in both headers and filename
  if (combinedText.includes('value score') || combinedText.includes('value ranking') || combinedText.includes('value') || combinedText.includes('value stocks')) {
    console.log('Detected category: Value');
    return 'Value';
  }
  if (combinedText.includes('growth score') || combinedText.includes('growth ranking') || combinedText.includes('growth') || combinedText.includes('growth stocks')) {
    console.log('Detected category: Growth');
    return 'Growth';
  }
  if (combinedText.includes('quality score') || combinedText.includes('quality ranking') || combinedText.includes('quality') || combinedText.includes('quality stocks') || combinedText.includes('quality_percentile')) {
    console.log('Detected category: Quality');
    return 'Quality';
  }
  if (combinedText.includes('momentum score') || combinedText.includes('momentum ranking') || combinedText.includes('momentum') || combinedText.includes('momentum stocks')) {
    console.log('Detected category: Momentum');
    return 'Momentum';
  }
  
  // Check for sector-based rankings in filename
  if (combinedText.includes('consumer defensive') || combinedText.includes('consumer staples') || combinedText.includes('defensive')) {
    console.log('Detected category: Consumer Defensive');
    return 'Consumer Defensive';
  }
  if (combinedText.includes('consumer cyclical') || combinedText.includes('consumer discretionary') || combinedText.includes('cyclical')) {
    console.log('Detected category: Consumer Cyclical');
    return 'Consumer Cyclical';
  }
  if (combinedText.includes('financial') || combinedText.includes('banking') || combinedText.includes('finance')) {
    console.log('Detected category: Financial');
    return 'Financial';
  }
  if (combinedText.includes('technology') || combinedText.includes('tech')) {
    console.log('Detected category: Technology');
    return 'Technology';
  }
  if (combinedText.includes('healthcare') || combinedText.includes('biotech') || combinedText.includes('pharma')) {
    console.log('Detected category: Healthcare');
    return 'Healthcare';
  }
  if (combinedText.includes('energy') || combinedText.includes('oil') || combinedText.includes('gas')) {
    console.log('Detected category: Energy');
    return 'Energy';
  }
  if (combinedText.includes('industrial') || combinedText.includes('manufacturing')) {
    console.log('Detected category: Industrial');
    return 'Industrial';
  }
  if (combinedText.includes('real estate') || combinedText.includes('reit')) {
    console.log('Detected category: Real Estate');
    return 'Real Estate';
  }
  if (combinedText.includes('materials') || combinedText.includes('basic materials')) {
    console.log('Detected category: Materials');
    return 'Materials';
  }
  if (combinedText.includes('utilities') || combinedText.includes('utility')) {
    console.log('Detected category: Utilities');
    return 'Utilities';
  }
  
  console.log('No specific category detected, defaulting to: Momentum');
  // Default to Momentum if no specific category is detected
  return 'Momentum';
}
  ...
  const processedRows = dataRows
    ...
    .map((row, rowIndex) => {
      ...
      headers.forEach((header, index) => {
        ...
        // Check for date-based score columns first (e.g., "Score on 2025-10-13", "Score on 10/13/2025")
        if (!mappedKey) {
          const datePatterns = [
            /score\s+on\s+\d{4}-\d{2}-\d{2}/i,  // Score on 2025-10-13
            /score\s+on\s+\d{1,2}\/\d{1,2}\/\d{4}/i,  // Score on 10/13/2025 or Score on 1/5/2025
            /score\s+on\s+\d{1,2}-\d{1,2}-\d{4}/i,  // Score on 10-13-2025
            /score\s+\d{4}-\d{2}-\d{2}/i,  // Score 2025-10-13 (without "on")
            /score\s+\d{1,2}\/\d{1,2}\/\d{4}/i  // Score 10/13/2025 (without "on")
          ];
          
          for (const pattern of datePatterns) {
            if (pattern.test(header)) {
              mappedKey = 'previousMomentumScore';
              console.log(`    Date-based score column detected: "${header}" -> "${mappedKey}"`);
              break;
            }
          }
        }
        ...
        if (!mappedKey) {
          ...
          if (lowerHeader.includes('symbol') || lowerHeader.includes('ticker')) {
            ...
            // Check if it's a date-based score column (e.g., "Score on 2025-10-13")
            const datePatterns = [
              /score\s+on\s+\d{4}-\d{2}-\d{2}/i,
              /score\s+on\s+\d{1,2}\/\d{1,2}\/\d{4}/i,
              /score\s+on\s+\d{1,2}-\d{1,2}-\d{4}/i,
              /score\s+\d{4}-\d{2}-\d{2}/i,
              /score\s+\d{1,2}\/\d{1,2}\/\d{4}/i
            ];
            
            const isDateBased = datePatterns.some(pattern => pattern.test(lowerHeader));
            
            if (isDateBased || lowerHeader.includes('previous') || lowerHeader.includes('prior') || lowerHeader.includes('last') || lowerHeader.includes('7 days ago')) {
              mappedKey = 'previousMomentumScore';
              console.log(`    Partial match: previous score (date or explicit) -> "${mappedKey}"`);
