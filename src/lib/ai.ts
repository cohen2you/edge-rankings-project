...
export async function generateArticle(stocks: EdgeRankingData[], rankingCategory: string = 'Momentum'): Promise<ArticleData> {
  try {
    const response = await fetch('/api/generate-article', {
      ...
      body: JSON.stringify({ stocks, rankingCategory }),
    });
    ...
    return generateTemplateArticle(stocks, rankingCategory);
  }
}

function generateTemplateArticle(stocks: EdgeRankingData[], rankingCategory: string = 'Momentum'): ArticleData {
  ...
  return {
    title: `${rankingCategory} Stocks To Add To Your Watchlist`,
...
function generateMarketContext(sector: string, stocks: EdgeRankingData[]): string {
  ...
  let context = "Broader market momentum is fading. ";
  
  switch (sector) {
    case 'solar':
      context += "A group of solar names are bucking the trend.";
      break;
    case 'technology':
      context += "Tech stocks are showing resilience.";
      break;
    case 'financial':
      context += "Financial stocks are gaining momentum.";
      break;
    default:
      context += "A group of stocks are bucking the trend.";
  }
...
function generateTechnicalAnalysis(stock: EdgeRankingData): string {
  ...
  if (stock.fiftyDayMA && stock.price) {
    ...
    analysis.push(`The stock is ${aboveMA ? 'above' : 'below'} its 50-day moving average of $${stock.fiftyDayMA.toFixed(2)}.`);
    analysis.push(`This indicates a ${aboveMA ? 'bullish' : 'bearish'} trend.`);
  }
  
  if (stock.hundredDayMA && stock.twoHundredDayMA) {
    analysis.push(`The 100-day and 200-day moving averages are at $${stock.hundredDayMA.toFixed(2)} and $${stock.twoHundredDayMA.toFixed(2)}, respectively.`);
  }
  ...
  if (stock.rsi) {
    ...
    analysis.push(`The RSI is calculated at ${stock.rsi.toFixed(2)}.`);
    analysis.push(`This ${rsiAnalysis}.`);
  }
...
function generateFundamentalAnalysis(stock: EdgeRankingData): string {
  ...
  if (stock.peRatio) {
    analysis.push(`The P/E ratio is ${stock.peRatio.toFixed(2)}.`);
  }
