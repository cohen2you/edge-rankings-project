...
export function generateFundamentalAnalysis(stockData: BenzingaStockData, edgeData: BenzingaEdgeData): string {
  ...
  if (edgeData.edgeScore > 0) {
    if (edgeData.edgeScore > 80) {
      analysis += `Excellent overall Edge score of ${edgeData.edgeScore.toFixed(1)}/100 indicates strong fundamentals across all metrics. `;
    } else if (edgeData.edgeScore > 60) {
      analysis += `Good Edge score of ${edgeData.edgeScore.toFixed(1)}/100 shows solid fundamental strength. `;
    } else {
      analysis += `Below-average Edge score of ${edgeData.edgeScore.toFixed(1)}/100 suggests fundamental challenges. `;
    }
    
    // Detailed Edge Score Breakdown - only if we have real component scores
    if (edgeData.momentumScore > 0 || edgeData.valueScore > 0 || edgeData.growthScore > 0 || edgeData.qualityScore > 0) {
      analysis += `Edge breakdown: Momentum ${edgeData.momentumScore.toFixed(1)}/100, Value ${edgeData.valueScore.toFixed(1)}/100, Growth ${edgeData.growthScore.toFixed(1)}/100, Quality ${edgeData.qualityScore.toFixed(1)}/100. `;
    }
